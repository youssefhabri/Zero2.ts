import { MessageEmbed, MessageEmbed as Embed } from 'discord.js';
import { KlasaMessage, ReactionHandler, RichDisplayEmojisObject, RichDisplayRunOptions } from 'klasa';
...
export class RichDisplay {
   ...
   * @since 0.4.0
   * @param {MessageEmbed} [embed=new MessageEmbed()] A Template embed to apply to all pages
   */
   ...
   * @since 0.4.0
   * @param {(Function|MessageEmbed)} embed A callback with the embed template passed and the embed returned, or an embed
   * @returns {this}
   * @chainable
   */
  addPage(embed) {
   ...
   */
  async run(message: KlasaMessage, options: RichDisplayRunOptions = {}): Promise<ReactionHandler> {
  ...
  async _footer() {
    for (let i = 0; i < this.pages.length; i++) {
      const footerText = this.pages[i].footer !== undefined ? ' | ' + this.pages[i].footer.text : '';
      this.pages[i].setFooter(`${this.footerPrefix}${i + 1}/${this.pages.length}${footerText}${this.footerSuffix}`);
    }
